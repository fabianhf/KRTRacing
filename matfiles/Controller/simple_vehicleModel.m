function [v_dot, beta_dot, psi_dot_dot,n_wheel,M_wheel,T_M] = simple_vehicleModel(v,beta,psi_dot,delta,fB,zeta,phi)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Simple Vehicle Model 1
v_max = 30;
P = 10000;
m = 1000;
a = (v_max - v) * P / m * phi; 
% a = phi
% x_dot_dot/a = sin(zeta * 2 * pi): Für zeta = 0.5 => x_dot_dot = a 
% y_dot_dot/a = cos(zeta * 2 * pi): Für zeta > 0.5 => y_dot_dot < 0
% delta gives the distibution of the acceleration between x and y)
v_dot = a * sin(zeta * 2 * pi);    % v_dot = x_dot_dot
beta_dot = a * cos(zeta * 2 * pi); % beta_dot = y_dot_dot
psi_dot_dot = 0;

%% Simple Vehicle Model 2
v_max = 30;
P = 10000;
m = 1000;
a = (v_max - v) * P / m * phi; 
