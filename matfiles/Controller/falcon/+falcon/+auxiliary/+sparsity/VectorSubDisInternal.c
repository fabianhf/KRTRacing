...
void mexFunction( int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[] ) {
    ...
    // ... h
    nh      = mxGetM(prhs[5])*mxGetN(prhs[5]);
    ...
    // ... h
    h       = mxGetPr(prhs[4]);
    numh    = mxGetM(prhs[4]) * mxGetN(prhs[4]);
    indh    = (uint32_T*)mxGetData(prhs[5]);
    nh      = mxGetM(prhs[5])*mxGetN(prhs[5]);
    ...
    for (i = 0; i < nG; i++)
    {
        ...
        {
            G[indG[i]-1] = G[indG[i]-1] - J[indJ[i]-1] * h[indh[i]-1];
        }
        else
        {
            mexPrintf("indG = %i, numG = %i, indJ = %i, numJ = %i\n, indh = %i, numh = %i",
                    indG[i], numG, indJ[i], numJ, indh[i], numh);
            mexErrMsgIdAndTxt( "falcon:SparsitySorting",
