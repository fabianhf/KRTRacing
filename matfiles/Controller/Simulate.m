...
controls = [...
    falcon.Control('delta', -0.1,   0.1,    1e1,  'fixed', true);...
    falcon.Control('fB',    0,      15000,  1e-4, 'fixed', true);...
    falcon.Control('zeta',  0,      1,      1,    'fixed', true);...
    falcon.Control('phi',   0,      1,      1,    'fixed', true);...
...
sKr = ones(size(tau)) * 0.05;
controlgrid = phase.addNewControlGrid(controls,tau);

% Set initial conditions for fb and C
controlgrid.setSpecificValues(controls(5), tau, sKr); % Works but seams logically wrong
controlgrid.setSpecificValues(controls([1:5]), [0; 0; 0.5; 0.6; 0.01]); % Works but seams logically wrong


% Set Boundary Condition
phase.setInitialBoundaries(states(:),[0 29.06 zeros(1,5)]');
...
solver.Options.MajorIterLimit = 200;
